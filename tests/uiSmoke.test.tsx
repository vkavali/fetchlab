...
describe('client route smoke', () => {
  ...
  it('renders the landing page', async () => {
    renderAt('/');
    expect(await screen.findByRole('heading', { level: 1, name: /Turn a customer problem into a reviewed pull request/i })).toBeTruthy();
  ...
  it('renders the how-to page', async () => {
    ...
    expect(screen.getByText('Turn customer evidence into a reviewed draft pull request')).toBeTruthy();
  ...
  it('renders the AI how-to page', async () => {
    ...
    expect(screen.getByText('Turn a customer problem into a reviewed draft pull request')).toBeTruthy();
  ...
  it('renders the enterprise page', async () => {
    renderAt('/enterprise');
    expect(await screen.findByRole('heading', { level: 1, name: 'Turn customer evidence into reviewed code.' })).toBeTruthy();
  ...
  it('renders the app shell', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'API Lab' }));
    await waitFor(() => {
  ...
  it('opens the correct auth mode and lets local users return', async () => {
    renderAt('/app');
    expect(await screen.findByRole('heading', { name: 'What product outcome should FetchLab own?' })).toBeTruthy();

    fireEvent.click(screen.getByRole('button', { name: 'Sign in' }));
    expect(await screen.findByRole('heading', { name: 'Sign in to FetchLab' })).toBeTruthy();

    fireEvent.click(screen.getByRole('button', { name: 'Back to local workspace' }));
    expect(await screen.findByRole('heading', { name: 'What product outcome should FetchLab own?' })).toBeTruthy();

    fireEvent.click(screen.getByRole('button', { name: 'Sign up' }));
    expect(await screen.findByRole('heading', { name: 'Create your FetchLab account' })).toBeTruthy();
  });
});
